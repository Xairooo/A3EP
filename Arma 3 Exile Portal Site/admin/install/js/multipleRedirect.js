$(function(){
	...
	var redirectStep = function( infoBox, url, baseurl ) {
		...
		$.ajax( url, {
			dataType: 'json',
			cache:	  false,
			timeout:  a3_TIMEOUT !== false ? parseInt( a3_TIMEOUT * 1000 ) : 30000,
			complete: function( jqXHR, textStatus )
			{
				...
				if ( response[2] ) {
					infoBox.children('.Redirect_loading').hide();
					infoBox.children('.Redirect_progress').show().find('.ProgressBar_progress').css({ width: ( response[2] + '%' ) });
				}
	...
	$('.MultipleRedirect').each(function(){
		redirectStep( $(this).children('.no_js_hide'), $(this).attr('data-url') + '&mr=1', $(this).attr('data-url') );
	});
})