...
class steamauthOOP {
    private $settings = array(
        "apikey" => $settingClass->getSteamapi(), // Get yours today from http://steamcommunity.com/dev/apikey
        "domainname" => $INFO['base_url'], // Displayed domain in the login-screen
        "loginpage" => $INFO['base_url']."/extra/steamAuth/a3srv.php", // Returns to last page if not set
        "logoutpage" => $INFO['base_url'],
    ...
    function loginUrl()
    {
        ...
        $slate = http_build_query($params, '', "&");
        $decoded_url = rawurldecode($slate);
        return 'https://steamcommunity.com/openid/login' . '?' . $decoded_url ;
    }
